...
// function _createBook(title, price = 30) {
...
function getNextBookId(bookId) {
    return storageService.query(BOOK_KEY)
        .then((book) => {
            let bookIdx = book.findIndex(book => book.id === bookId)
            if (bookIdx === book.length - 1) bookIdx = -1
            return book[bookIdx + 1].id
        })
}
...
function getPrevBookId(bookId) {
    return storageService.query(BOOK_KEY)
        .then((book) => {
            let bookIdx = book.findIndex(book => book.id === bookId)
            console.log('bookId', bookIdx)
            if (!bookIdx) return
            return book[bookIdx - 1].id
        })
}

function getEmptyBook(title = '', listPrice = { amount: 0 }, description = '') {
    return { id: '', title, listPrice, description }
}

function getEmptyReview(fullName = '', txt = '', rating = '1', readAt = new Date()) {
    return { fullName, txt, rating, readAt }
}
